...
public class P2Winnow implements Model, SimilarityComputable<P2Winnow>  {
	 ...
	 * @param numberOfClasses osztalyok szama
	 */
	public P2Winnow(SparseVector w, int numberOfClasses, double eta, double nrOAtt) {
		this.numberOfClasses = numberOfClasses;
	...
	public Object clone(){
		return new P2Winnow(w, numberOfClasses, eta, attrnum);
	}
	...
	public void update(SparseVector instance, double label) {
		...
		if ( y != y_pred ) {	//teves osztalyozas eseten javitjuk a sulyt
			if ( y == 1.0 ) {	//false negativ esetben
				//w.add(w, (1+eta));	//szorozzuk (1+eps) -al
				for ( VectorEntry ve : instance ) {
					if ( w.get(ve.index) == 0.0 ) {
						w.put(ve.index, Math.exp(eta*y*ve.value));
					} else {
	...
	public double predict(SparseVector instance) {
		...
		double n = attrnum/4;		//a jellemzok szama, ez lesz alapbol a kuszobertek
